...
##' @param formula specify which 2 columns to call \code{\link[base]{table}} on: \code{pert_type ~ pert_itime} or \code{type ~ pert_type} to tabulate single column
...
##' @param pdata (optional) data.table, the output of \code{\link{openpData}}. By default \code{perturbTable} reads Cmap file (path provided in CMap_files), for multiple test it can be useful to supply data.table.
...
perturbTable = function(CMap_files, formula = pert_type ~ pert_itime, pert_ids = NULL, pdata = NULL) {

  if(is.null(pdata)) pdata = openpData(CMap_files)
